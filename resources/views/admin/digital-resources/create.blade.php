...
@section('content')
<h2 class="mb-3">Tambah Artikel & Jurnal</h2>

<div class="card app-card">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.digital-resources.store') }}" class="row g-3">
            @csrf
            <div class="col-md-8">
            ...
            <div class="col-md-4">
                ...
                <input type="number" class="form-control @error('year') is-invalid @enderror" name="year" value="{{ old('year') }}" min="1900" max="2100">
                @error('year')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-md-6">
                <label class="form-label">Penulis</label>
                <input class="form-control @error('author') is-invalid @enderror" name="author" value="{{ old('author') }}">
                @error('author')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-md-6">
                <label class="form-label">Topik</label>
                <input class="form-control @error('topic') is-invalid @enderror" name="topic" value="{{ old('topic') }}">
            ...
            <div class="col-md-6">
                ...
                <select class="form-select @error('access_level') is-invalid @enderror" name="access_level" required>
                    <option value="open" {{ old('access_level') === 'open' ? 'selected' : '' }}>open</option>
                    <option value="limited" {{ old('access_level') === 'limited' ? 'selected' : '' }}>limited</option>
                </select>
            ...
            <div class="col-12">
                ...
                <input class="form-control @error('resource_url') is-invalid @enderror" name="resource_url" value="{{ old('resource_url') }}">
                @error('resource_url')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-12">
                <label class="form-label">Abstrak</label>
                <textarea class="form-control @error('abstract') is-invalid @enderror" rows="4" name="abstract">{{ old('abstract') }}</textarea>
